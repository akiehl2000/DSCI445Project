...
for(k in 1:K) {
 ...
 bag_fit = randomForest(PassYds ~ . - PassTD - PassInt - FL - PassDrops - BadThrow - BadPer - YdsLost - Sk - FirstDPassPer, data = trn, mtry = ncol(trn)-10, importance = TRUE)
}
...
for(k in 1:K) {
 ...
 bag_fit = randomForest(PassInt ~ . - PassTD - PassYds - FL, data = trn, mtry = ncol(trn)-4, importance = TRUE)
}
importance(bag_fit)
...
for(k in 1:K) {
 ...
 bag_fit = randomForest(PassTD ~ . - PassYds - PassInt - FL, data = trn, mtry = ncol(trn)-4, importance = TRUE)
}
...
for(k in 1:K) {
 ...
 bag_fit = randomForest(FL ~ . - PassYds - PassInt - PassTD, data = trn, mtry = ncol(trn)-4, importance = TRUE)
}
...
randomForest(PassYds ~ . - PassTD - PassInt - FL - PassDrops - BadThrow - BadPer - YdsLost - Sk - FirstDPassPer, data = train_data, mtry = ncol(train_data)-10, importance = TRUE) %>%
...
randomForest(PassInt ~ . - PassTD - PassYds - FL, data = train_data, mtry = ncol(train_data)-4, importance = TRUE) %>%
