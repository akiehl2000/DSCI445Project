...
# cv_error = rep(NA, deg)
# for(i in 1:deg) {
#   poly_passyards = glm(PassYds ~ poly(Cmp,i) + poly(PassAtt,i) + poly(Sk,i) + poly(YdsLost,i) + poly(PassLng,i) + poly(Rate,i) + poly(FirstDPass,i) + poly(FirstDPassPer,i) + poly(CAY,i) + poly(YACPerCmp,i) + poly(PassDrops,i) + poly(BadThrow,i) + poly(BadPer,i), data = train_data)
#   cv_error[i] = cv.glm(PassYds, poly)$delta[1]
# }
```
...
pcplot %>%
  saveRDS('./Plots/pcplot.rds')

pca_scores = data.frame(pca_model$x)
data.frame(pca_model$rotation[, 1:3])
...
ggplot() + 
  ...
  labs(title = "CV Decision Tree Deviance", x = "Terminal Node", y = "Error")
...
for(k in 1:K) {
 ...
 bag_fit = randomForest(PassYds ~ . - PassTD - PassInt - FL - PassDrops - BadThrow - BadPer - YdsLost - Sk - FirstDPassPer - Rate, data = trn, mtry = ncol(trn)-11, importance = TRUE)
}
importance(bag_fit)
...
for(k in 1:K) {
 ...
 bag_fit = randomForest(PassInt ~ . - PassTD - PassYds - FL - PassDrops - Sk - YdsLost - YACPerCmp - BadPer - PassAtt, data = trn, mtry = ncol(trn)-10, importance = TRUE)
}
importance(bag_fit)
...
for(k in 1:K) {
 ...
 bag_fit = randomForest(PassTD ~ . - PassYds - PassInt - FL - Sk - YACPerCmp - YdsLost - FirstDPassPer - PassDrops, data = trn, mtry = ncol(trn)-9, importance = TRUE)
}
importance(bag_fit)
...
randomForest(PassYds ~ . - PassTD - PassInt - FL - PassDrops - BadThrow - BadPer - YdsLost - Sk - FirstDPassPer - Rate, data = train_data, mtry = ncol(trn)-11, importance = TRUE) %>%
...
randomForest(PassInt ~ . - PassTD - PassYds - FL - PassDrops - Sk - YdsLost - YACPerCmp - BadPer - PassAtt, data = train_data, mtry = ncol(trn)-10, importance = TRUE) %>%
