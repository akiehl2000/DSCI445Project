...
data.frame(Var = as.vector(labels(import)[[1]]), Purity = as.vector(import)) %>%
  ...
  saveRDS('./Plots/recYdsPlot.rds')

model <- randomForest(RecTD ~ . - Rec - RecYds - FL, data = train_data, mtry = 15, ntree = 10)
import <- importance(model)
