...
Exploratory coorelation matrix for four response variables of interest. 
```{r}
ggpairs(train_data[,c(2:4, 7)])
```

Correlation matrix used to identify multicollinearity. Evidence of high correlation found between `FirstDRec` and `Tgt` (0.75), `RecYBC` and `FirstDRec` (0.77), `ADOT` and `YBCPerR` (0.86), and `DropPerRec` and `RecDrop` (0.84). 
```{r}
VIF(lm(Rec ~ . - RecYds - RecTD - FL, train_data))

source("http://www.sthda.com/upload/rquery_cormat.r")
rquery.cormat(train_data[,-c(2:4, 7)])$r

model <- lm(Rec ~ . - RecYds - RecTD - FL - FirstDRec - RecYBC - YBCPerR - DropPerRec - YACPerR, train_data)
VIF(model)
summary(model)
plot(model)
```
...
Rec: Tgt, RecLng, FirstDRec, RecDrop, Rat
RecYds: Tgt, RecLng, RecYBC, YBCPerR, RecYAC, YACPerR, ADOT, RecDrop, Rat
RecTD: RecYBC, ADOT, RecBrkTkl, Rat
FL: Fmb
```{r}
plot(train_data$Tgt, train_data$Rec)
plot(train_data$RecLng, train_data$Rec) # poly
plot(train_data$RecLng, train_data$RecYds) #poly
plot(train_data$YBCPerR, train_data$RecYds) # poly
```
...
best_subset <- regsubsets(Rec ~ . - RecYds - RecTD - FL - FirstDRec - RecYBC - YBCPerR - DropPerRec - YACPerR, train_data, nvmax = 10)
...
Best subset selection performed with `RecYds` as response. Some polynomial terms included. 
```{r}
best_subset <- regsubsets(RecYds ~ . - Rec - RecTD - FL - RecYAC, data = train_data, nvmax = 15)
best_subset <- regsubsets(RecYds ~ . + I(RecLng^2) + I(RecYBC^2) - Rec - RecTD - FL - FirstDRec - RecYAC - YBCPerR - DropPerRec - YACPerR, train_data, nvmax = 10)
best_subset_sum <- summary(best_subset)
...
best_subset <- regsubsets(RecTD ~ . - Rec - RecYds - FL - FirstDRec - RecYAC - YBCPerR - DropPerRec - YACPerR, train_data, nvmax = 10)
best_subset_sum <- summary(best_subset)
...
best_subset <- regsubsets(FL ~ . - Rec - RecYds - RecTD - FirstDRec - RecYAC - YBCPerR - DropPerRec - YACPerR, train_data, nvmax = 10)
best_subset_sum <- summary(best_subset)
...
```{r}
x <- model.matrix(RecYds ~ . - Rec - RecTD - FL - RecYAC, train_data)
y <- train_data$RecYds
...
for (k in 1:K) {
  ...
  x <- model.matrix(RecYds ~ . - Rec - RecTD - FL - RecYAC, trn)
  y <- trn$RecYds
  
  lasso_model <- glmnet(x, y, lambda = exp(1))
  
  pred_x <- model.matrix(RecYds ~ . - Rec - RecTD - FL - RecYAC, vld)
  pred <- round(predict(lasso_model, pred_x))
...
ggplot() +
  ...
  geom_point(aes(5, cv_tree_model$dev[which(cv_tree_model$size == 5)]), color = 'red') +
...
ggplot() +
  geom_line(aes(cv_tree_model$size, cv_tree_model$dev)) +
  geom_point(aes(7, cv_tree_model$dev[which(cv_tree_model$size == 7)]), color = 'red') +
  scale_x_discrete(limits = factor(1:10)) +
  labs(title='CV Decision Tree Deviance', x='Terminal Nodes', y='Error')

tree_model <- prune.tree(tree_model, best = '7')
summary(tree_model)
...
for (k in 1:K) {
  ...
  tree_model <- prune.tree(tree(RecYds ~ . - Rec - RecTD - FL, trn), best = '7')
...
ggplot() +
  geom_line(aes(cv_tree_model$size, cv_tree_model$dev)) +
  geom_point(aes(2, cv_tree_model$dev[which(cv_tree_model$size == 2)]), color = 'red') +
  scale_x_discrete(limits = factor(1:10)) +
  labs(title='CV Decision Tree Deviance', x='Terminal Nodes', y='Error')
...
for (lambda in results$lambda) {
  ...
  for (k in 1:K) {
    ...
    model <- gbm(RecYds ~ . - Rec - RecTD - FL, data = trn, distribution = 'gaussian', n.trees = 100, shrinkage = lambda)
...
for (k in 1:K) {
  ...
  model <- randomForest(Rec ~ . - RecYds - RecTD - FL, data = trn, mtry = 15, ntree = 10)
...
for (k in 1:K) {
  ...
  model <- randomForest(RecYds ~ . - Rec - RecTD - FL, data = trn, mtry = 15, ntree = 10)
...
for (k in 1:K) {
  ...
  model <- randomForest(RecTD ~ . - Rec - RecYds - FL, data = trn, mtry = 15, ntree = 10)
...
for (k in 1:K) {
  ...
  model <- randomForest(FL ~ . - Rec - RecYds - RecTD - FL, data = trn, mtry = 15, ntree = 10)
...
for (k in 1:K) {
  ...
  model <- randomForest(Rec ~ . - RecYds - RecTD - FL, data = trn, mtry = 4, ntree = 10)
...
for (k in 1:K) {
  ...
  model <- randomForest(RecYds ~ . - Rec - RecTD - FL, data = trn, mtry = 4, ntree = 10)
...
for (k in 1:K) {
  ...
  model <- randomForest(RecTD ~ . - Rec - RecYds - FL, data = trn, mtry = 4, ntree = 10)
...
for (k in 1:K) {
  ...
  model <- randomForest(FL ~ . - Rec - RecYds - RecTD - FL, data = trn, mtry = 4, ntree = 10)
...
Save best models as .rds files.
```{r, warning=FALSE}
# Random forest model chosen for Rec
randomForest(Rec ~ . - RecYds - RecTD - FL, data = train_data, mtry = 4, ntree = 200) %>%
  saveRDS('./Models/recModel.rds')

# Bagged tree model chosen for RecYds
randomForest(RecYds ~ . - Rec - RecTD - FL, data = train_data, mtry = 15, ntree = 200) %>%
  saveRDS('./Models/recYdsModel.rds')

# Bagged tree model chosen for RecTD
randomForest(RecTD ~ . - Rec - RecYds - FL, data = trn, mtry = 15, ntree = 100) %>%
  saveRDS('./Models/recTDModel.rds')

# Tree-based model chosen for FL
prune.tree(tree(FL ~ . - Rec - RecYds - RecTD, train_data), best = '2') %>%
