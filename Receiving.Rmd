...
for (k in 1:K) {
  ...
  tree_model <- prune.tree(tree(RecYds ~ . - Rec - RecTD - FL - RecYAC, trn), best = '7')
...
for (k in 1:K) {
  ...
  model <- randomForest(Rec ~ . - RecYds - RecTD - FL, data = trn, mtry = 15, ntree = 50)
...
for (k in 1:K) {
  ...
  model <- randomForest(RecYds ~ . - Rec - RecTD - FL - RecYAC, data = trn, mtry = 14, ntree = 50)
...
for (k in 1:K) {
  ...
  model <- randomForest(RecTD ~ . - Rec - RecYds - FL, data = trn, mtry = 15, ntree = 50)
...
for (k in 1:K) {
  ...
  model <- randomForest(FL ~ . - Rec - RecYds - RecTD - FL, data = trn, mtry = 15, ntree = 50)
...
randomForest(Rec ~ Tgt + FirstDRec + RecYAC + RecYBC + Rat, data = train_data, mtry = 2, ntree = 200) %>%
...
# Decision tree model chosen for RecTD
prune.tree(tree(RecTD ~ . - Rec - RecYds - FL, train_data), best = '2') %>%
